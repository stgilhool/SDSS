...
lsf_coeff_arr = dblarr(26, napofiles)
...
if n_elements(ex.epsilon) eq 0 then epsilon=0.6 else epsilon=ex.epsilon
...
psopen, 'limb_darkening_demo.eps', /encapsulated, xs=10, ys=10, /inches, /color

!p.multi=[0,1,3]

; Get the rotational broadening
vsinivec = [5d0, 10d0, 20d0]
; LD coeffs
ep_vec = [0.3, 0.4, 0.5, 0.6, 0.7]
ep_str = ['0.3', '0.4', '0.5', '0.6', '0.7']

co_vec = [!yellow, !orange, !red, !green, !blue]

foreach vsini, vsinivec, vidx do begin

    foreach ep, ep_vec, epidx do begin
        
        ex = {epsilon:ep}
        lsfrot = make_broadening_kernel(vsini, oversamp=oversamp, _EXTRA=ex)
...
;           ;title="Vsini = "+strtrim(fix(vsini),2)+" km/s", charsize=2, linest=2, $
        ...
        if epidx eq 0 then begin
            plot, lsf_rotx, lsf_rot, xr = [-5,5], yr = [0, 1.1*max(lsf_rot)], $
              charsize=2.5, xtit="Pixel", tit='Broadening Kernel for Vsini = '+strtrim(fix(vsini),2)+' km/s color-coded by '+textoidl('\epsilon'), /nodata
            al_legend, ep_str, colors=co_vec, psym=8
        endif
        oplot, lsf_rotx, lsf_rot, thick=7
        oplot, lsf_rotx, lsf_rot, thick=3, color=co_vec[epidx]
    endforeach
