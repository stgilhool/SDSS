...
density, teff_realizations[*,young_star_idx], veq_realizations[*,young_star_idx], $
  /ylog, yr=[1,300], xr=[4000,2600], /ys, /xs, /dlog, drange=[5,1000], ct=-1, $
...
for tbin = 0, ntbins-1 do begin
    ...
    if yconf_int[0] ge 1 then begin
        ; Regular CI's
        ; line
        oplot, replicate(xtemp,2), yconf_int, color=400, thick=6
        ; hats
        oplot, [-5,5]+xtemp, replicate(yconf_int[0],2), color=400, thick=6
        oplot, [-5,5]+xtemp, replicate(yconf_int[1],2), color=400, thick=6
        plotsym, 8, /fill
        ; mode
        oplot, [xtemp], [yconf_mode], color=400, ps=8, thick=6
    endif else begin
        ; Lower limits
        plotsym, 5, /fill
        ...
        oplot, replicate(xtemp,2), yconf_int, color=400, thick=6
        ; arrow
        oplot, [xtemp], [1.1d0], color=400, ps=8, thick=6
    endelse
endfor


density, teff_realizations[*,old_star_idx], veq_realizations[*,old_star_idx], /ylog, $
  yr=[1,300], xr=[4000,2600], /ys, /xs, /dlog, drange=[5,1000], ct=-1, $
...
for tbin = 0, ntbins-1 do begin
    ...
    if oconf_int[0] ge 1 then begin
        ; Regular CI's
        ; line
        oplot, replicate(xtemp,2), oconf_int, color=400, thick=6
        ; hats
        oplot, [-5,5]+xtemp, replicate(oconf_int[0],2), color=400, thick=6
        oplot, [-5,5]+xtemp, replicate(oconf_int[1],2), color=400, thick=6
        plotsym, 8, /fill
        ; mode
        oplot, [xtemp], [oconf_mode], color=400, ps=8, thick=6
    endif else begin
        ; Lower limits
        plotsym, 5, /fill
