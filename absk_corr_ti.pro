...
window, 0, xs = 1700, ys = 1000, title = "EW vs M_K for Ti lines"
!p.multi = [0,3,2]
for lnum = 0, nti-1 do begin
    ew = ti_info[lnum].ew[plx_idx]
    fnum = strtrim(ti_info[lnum].ti_num,2)
    wl_ctr = ti_info[lnum].wl_ctr
    plot, absk_vec, ew, $
      title = "Ti line num "+fnum+" at wl = "+strtrim(wl_ctr,2), $
      ps=6, charsize = 1.5, xtit = "M_K", ytit = "EW" 
endfor
stop
