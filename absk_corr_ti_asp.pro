...
; Read in aspcap spectra
nspec = 1350L
goto, skipreadin
aspcap = readin_aspcap(hdu=3, nfiles=nspec)
save, /variables, filename="readin_aspcap3.sav"
skipreadin:
restore, "readin_aspcap3.sav"

spectra = aspcap.spectra
errors  = aspcap.errors
masks = intarr(size(spectra, /dim)) ;Initialize to 0

wl1 = 6d-6
log_wl = aspcap[0].logwl_grid
wl_grid = aspcap[0].wl_grid
...
; Check that aspcap order is kosher
; for spec = 0, nspec-1 do begin
;     asphead = *(aspcap[spec].header)
;     aspid = fxpar(asphead, 'OBJ')
;     aspid = strtrim(aspid,2)
;     if info[spec].objid ne aspid then message, "aspcap order is messed up" $
;       else begin
;         print, info[spec].objid
;         print, aspid
;     endelse
; endfor
...
feh_vec = info.feh_irtf
efec_vec = info.efeh_irtf
...
; store their ew's
eq_width_all = ti_info.ew
eq_width_all = transpose(eq_width_all)
; choose which lines to use 
linenums = 0
n_ti = n_elements(linenums) 
; now whittle down the ew array
; just the columns corresponding to the ti line(s) we want
eq_width_col = eq_width_all[linenums,*]
; add a column of 1's for constant term
eq_width_mtx = [reform(replicate(1d0,nspec),1,nspec), eq_width_col]
; keep only the ones with M_k's
eq_width_plx = eq_width_mtx[*,plx_idx]
; now axe any rows with NaNs
fit_row_idx = where(total(finite(eq_width_plx), 1) eq n_ti+1, nfit)

if nfit eq 0 then message, "no EW's were fit"

eq_width = eq_width_plx[*,fit_row_idx]

absk_vec = absk_vec[plx_idx[fit_row_idx]]
eabsk_vec = eabsk_vec[plx_idx[fit_row_idx]]
...
plot, reform(eq_width[1,*]), absk_model, ps=6, $
  ...
  ytit='M_K(fit)', xtit='EW', charsize = 1.5
