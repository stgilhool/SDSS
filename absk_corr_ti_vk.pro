...
; Save arrays
for linenum = 0, n_ti_lines-1 do begin
...
vmag_vec = info.vmag_apass
vk_vec = vmag_vec - k_vec

both_idx = where(finite(plx_vec) and finite(vk_vec), nboth)
vkboth_vec = vk_vec[both_idx]
plxboth_vec = plx_vec[both_idx]
e_plxboth_vec = e_plx_vec[both_idx]
kboth_vec = k_vec[both_idx]
; Get their absolute k magnitudes
;abskboth_vec = kboth_vec + 5d0 + 5d0*alog10(plxboth_vec)
;eabskboth_vec = 5d0*0.434*(e_plxboth_vec/plxboth_vec)
abskboth_vec = absk_vec[both_idx]
eabskboth_vec = eabsk_vec[both_idx]
...
vk_vec = vkboth_vec[fit_idx]
vk_vec = reform(vk_vec, 1, nfit)
...
eq_width = [reform(replicate(1d0, nfit),1,nfit), eq_width, vk_vec]
help, eq_width
...
plot, reform(eq_width[1,*]), absk_model, ps=6, $
  title="M_K(fit) vs. EW | Red pts are data", $
  xtit='EW', ytit='M_K(fit)', charsize = 1.5
