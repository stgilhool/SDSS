...
plx_idx = where(finite(info.feh_irtf), nplx)
;plx_vec = info[plx_idx].plx_dtm/1d3
plx_vec = info[plx_idx].feh_irtf
;e_plx_vec = info[plx_idx].eplx_dtm/1d3
e_plx_vec = info[plx_idx].efeh_irtf
...
;absk_arr = rebin(reform(absk_vec,1,nplx),npix,162)
;mean_absk = mean(absk_arr, dimension=2)
;diff_absk = absk_arr - rebin(mean_absk, npix, nplx)
;diff_absk = absk_arr - rebin(mean_absk, npix, 162)
...
mwrfits, eq_width_fit, '../eq_width_unmasked.fits', /create
stop
...
;plot, absk_vec, reform(eq_width_fit[1,*]), xtit = "Fe/H", ytit = "EW", ps = 6, yr=[0,0.5]
;print, best_window_pos
;stop
