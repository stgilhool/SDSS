...
;testidx = where(finite(info.plx_dtm), ntest)
...
; Using previous results (from absk_newtry and before), we use the
; EW summing method and all calibration spectra to calculate MLR
; parameters, which we then use to calculate M_k's for all stars in
; the sample.
...
nspec_cal = n_elements(testidx) ; using all cal spectra
testidxidx = lindgen(nspec_cal) 
cal_idx = testidx
 
; Load those spectra
specinfo = mrdfits('/home/stgilhool/APOGEE/absk_corr/absk_corr_init.fits',1)
nspec = n_elements(specinfo) 
apo_idx = lindgen(nspec)
;specinfo = specinfo[testidx]

; find the indices of the calibration set in testidx
;cal_idx = value_locate(apo_idx, testidx_cal)  
;cal_idx = lindgen(nspec_cal)
...
;;;;;;;;;;;;;;;;
    ...
    feh_rgrss = specinfo.feh_rgrss
    feh_nan_idx = where(not finite(feh), nfehnan)
    if nfehnan ne 0 then begin
        feh[feh_nan_idx] = feh_rgrss[feh_nan_idx]
    endif
...
;;; Fe/H > 0.1
    ...
    fehhist = histogram(feh, binsize = bsize, omin=om, /nan)
...
ew_mean = mean(ew_arr_cal, dimension=2, /nan)

; make nan'ed feh values = 0
; feh_klooge_idx = where(not finite(feh))
; feh_klooge = feh
; feh_klooge[feh_klooge_idx] = 0.0
...
modsort = sort(absk_model)
modsort_cal = sort(absk_model_cal)

file_idx = lindgen(nspec)
;file_idx_cal = cal_idx[modsort_cal]
file_idx_cal = value_locate(absk_model[modsort], absk_model_cal[modsort_cal])
...
plot, file_idx, absk_vec[modsort], ps = 6, title=$
...
oploterror, file_idx_cal, absk_vec_cal[modsort_cal], eabsk_vec_cal[modsort_cal], ps = 6, color=99999
oplot, file_idx_cal, absk_vec_cal[modsort_cal], ps=6, color=99999
oplot, file_idx, absk_model[modsort], ps=6, color=200
...
; ;oploterror, filenum, absk_vec[aksort], eabsk_vec[aksort], ps=6
; ;oplot, filenum, absk_vec[aksort], ps=6
; oploterror, filenum_cal, absk_vec_cal[aksort_cal], eabsk_vec_cal[aksort_cal], ps = 6, color=99999
; oplot, filenum_cal, absk_vec_cal[aksort_cal], ps=6, color=99999
; oplot, absk_model[aksort], ps=6, color=200


outstr = {ABSK:absk_vec, $
          EABSK:eabsk_vec, $
          CAL_IDX:cal_idx, $
          EQ_WIDTH:eq_width, $
          ABSK_MODEL:absk_model}

mwrfits, outstr, 'absk_fitall_model.fits', /create
print, "Result structure written"
