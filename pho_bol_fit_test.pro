...
;;;MAKE LSF
;;;;;;;;;;;;;;;;;;;;;;;;;
function BOLmake_lsf, lsf_coeffs, x_min, x_max, nplsf, oversamp
; LSF stuff
...
function BOLfit_spectrum, free_pars, $
                       pho_wl=pho_wl, $
                       pho_spec=pho_spec, $
                       sdss_wl=sdss_wl, $
                       sdss_spec=sdss_spec, $
...
sdss_spec = sdss_spec_arr[*,0]
sdss_err = sdss_err_arr[*,0]

;apply mask
sdss_mask = mask_arr[*,0]
...
feh = 0.3
logg=5.0
pho_idx = where(pho_teff eq teff and pho_logg eq logg and pho_feh eq feh, nres)
if nres eq 1 then pho_spec=pho_arr[*,pho_idx] $
  else message, "No template match"
