...
for afil = 0, nafil-1 do begin
    ...
    apo_logwl = apo_logwl_raw[x_min:x_max]
    ...
    foreach teff, pteff, teff_idx do begin
        ...
        for feh = 0, nfeh-1 do begin
            ...
            pspec_norm = continuum_fit(dindgen(n_elements(pspec)), pspec, deg=3)
            ...
            ;oversample
            apo_logwl_over = interpol(apo_logwl, x, xx)
            ;pho_spec_over = interpol(pho_spec, apo_logwl,
            ;apo_logwl_over)
            pho_spec_over = interpol(pho_spec, x, xx)
            lsf_0 = xx[nxx/2]
            lx = (dindgen(nplsf)-(nplsf/2))/oversamp + lsf_0
            lsf_y = lsf_gh(lx, lsf_0, lsf_coeff)/oversamp
            ;plot, lx, lsf_y, yr=[0,0.05], chars=2
            ...
            mod_spec = convol(pho_spec_over, lsf_y, /edge_wrap)
            
            ;pspec = interpol(pho_cont, pwl_air, apo_wl)
            
            ; Define relevant spectral vectors
            
            pho_spec_down = downsample_tophat(mod_spec, oversamp)
            ...
            corr = c_correlate(obs, pho_spec_down, lag)
            !p.multi=[0,1,2]
            plot, lag, corr, /xs
            plot, obs, /xs
            oplot, pho_spec_down, color=200
            stop
                ...
                r = amoeba3(ftol, scale=scale, p0=guess, function_name='spec_fit', $
                            function_value=fval, nmax=20, ncalls=ncalls)
