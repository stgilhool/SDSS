...
function model_lines, x_min, x_max, amp, ctr, vsini 
...
;;;;;;;;;;;;;;;;;;;
sig = replicate(0.5d0, nlines)
newamp = replicate(1d0, nlines)
;gauss_pars = [transpose(amp), transpose(ctr), transpose(sig)]
gauss_pars = [transpose(newamp), transpose(ctr), transpose(sig)]
tau_arr = dblarr(nxx, nlines)

; make the gaussians
for i = 0, nlines-1 do begin
    ;tau_arr[*,i] = gaussian(xx, gauss_pars[*,i])
    gauss0 = gaussian(xx, gauss_pars[*,i])
    ; scale
    ew0 = total(gauss0, /double)
    ew_factor = amp[i]/ew0
    tau_arr[*,i] = gauss0 * ew_factor
...
function fit_function, p, X_MIN=x_min, X_MAX=x_max, LINES_PER_GROUP=lines_per_group, AMPIDX0=ampidx0, CTRIDX0=ctridx0, OBS_SPEC=obs_spec, OBS_ERR=obs_err, VIS=vis
...
for grp = 0, ngroups-1 do begin
    ...
    ctr = p[ctridx0[grp]:ctridx0[grp]+lines_per_group[grp]-1]

    if grp eq 0 then begin
        model_spec = model_lines(x_min[grp], x_max[grp], amp, ctr, vsini)
    endif else begin
        new_spec = model_lines(x_min[grp], x_max[grp], amp, ctr, vsini)
...
; and other data
apo_str = readin_apo(hdu=8, /irtf, nfiles=40, /flux)
...
ctr_guess = [3140, 3155, 3177, 3190, 3197]
x_min = [2840L]
x_max = [3200L]


spec_chunk = obs_spectrum[x_min:x_max]
error_chunk = obs_error[x_min:x_max]

xtemp = dindgen(((x_max[0]-x_min[0])+1))
!p.multi=0
s_norm = continuum_fit(xtemp, spec_chunk, degree=3, /display)
!p.multi=[0,1,2]
window, 0, xs = 1700, ys= 1000
plot, xtemp+x_min[0], spec_chunk


spec_chunk = temporary(spec_chunk)/s_norm
error_chunk = temporary(error_chunk)/s_norm

plot, xtemp+x_min[0], spec_chunk

;test
;test
apo_str = readin_apo(hdu=8, /irtf, nfiles=40)
obs_spectrum_norm = apo_str[fnum].spectra
spec_chunk_norm = obs_spectrum_norm[x_min:x_max]
oplot, xtemp+x_min[0], spec_chunk_norm, color=200
stop

;amp_guess = replicate(1d0, nlinestot)
amp_guess = [2d0, 0.5d0, 1.25d0]
;sig_guess = replicate(0.5d0, nlinestot)
vsini_guess = 10d0
;goto, skip_over
;ctr_guess = [5782.5, 5793, 5798]
;x_min = [5773L]
;x_max = [5809L]
...
ctridx0 = ampidx0 + nlinestot
ctridx1 = ampidx1 + nlinestot
...
parinfo[nlinestot:2*nlinestot-1].value = ctr_guess
...
parinfo[nlinestot:2*nlinestot-1].limits[0] = ctr_guess-1d0
parinfo[nlinestot:2*nlinestot-1].limits[1] = ctr_guess+1d0
...
print, "Ctr (fit): " + strtrim(r[nlinestot:2*nlinestot-1], 2)
