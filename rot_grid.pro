...
function model_observation, x_min, x_max, lsf_apo_coeff, pho_spec, tell_spec, vsini, tell_scale, spec_scale, norm0, norm1, QUICK=quick
...
pho_spec_over = interpol(pho_spec, apo_logwl, wl_log_over)
...
mod_obs = model_observation(x_min, x_max, lsf_coeff, pho_spec, tell_spec, $
                            vsini, tell_scale, spec_scale, norm0, norm1, /quick)
...
apo_logwl_trunc = apo_logwl[x_min:x_max]
...
;;; Read in PHOENIX
ppath = '/home/stgilhool/RV_projects/IRCS_rv/data/supplemental/stellar_template_models/'

pfeh = ['+1.0', '+0.5', '-0.0', '-0.5', '-1.0', '-1.5', '-2.0', '-3.0']
pwlfile = ppath + 'WAVE_PHOENIX-ACES-AGSS-COND-2011.fits'

pho_wl = mrdfits(pwlfile,0)
pho_logwl = alog10(pho_wl)

npfiles = n_elements(pfeh)
pfil = strarr(npfiles)
pspectra = dblarr(1569128, npfiles)
pspec = dblarr(n_elements(apo_logwl), npfiles)
px = dindgen(n_elements(apo_logwl))
for i = 0, npfiles-1 do begin
    pfil[i] = ppath+'lte03300-5.00'+pfeh[i]+'.PHOENIX-ACES-AGSS-COND-2011-HiRes.fits'
    pspectra[0,i] = mrdfits(pfil[i],0) 
    ; truncate phoenix spectra by splining onto apogee grid
    pspec_tmp = interpol(pspectra[*,i], pho_logwl, apo_logwl)
    pnorm = continuum_fit(px, pspec_tmp)
    pspec[0,i] = pspec_tmp/pnorm
endfor

npad = 5

vsini_vec = dindgen(40)/2. + 2d0
nafil = 20
npfil = n_elements(pfeh)

chiarr = dindgen(nafil, npfil, n_elements(vsini_vec))
for afil = 0, nafil-1 do begin
    for pfil = 0, npfil-1 do begin
        for v = 0, n_elements(vsini_vec)-1 do begin
            ...
            r = amoeba3(ftol, scale=scale, p0=guess, function_name='spec_fit', $
                        function_value=fval, nmax=10d4, ncalls=ncalls)
            
            
            chi2 = fval[0]
            
            chiarr[afil,pfil,v] = chi2
        endfor
    endfor
endfor
