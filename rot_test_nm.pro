...
rot_neg_idx = where(lsf_rot lt 0d0, ncnt, complement=rot_gd_idx)
if ncnt gt 0 then lsf_rot[rot_neg_idx]=0d0
; Normalize
lsf_rot = lsf_rot/total(lsf_rot, /double)

lsf_rot = lsf_rot[rot_gd_idx]
...
nvel = 40
nstrength = 20
strength_vec = (dindgen(nstrength)+1.5)*0.1d0
vsini_vec = dindgen(nvel)*0.5+4d0
co = dblarr(3, nvel, nstrength)

for w = 0, nstrength-1 do begin
amp_scaled = amp*strength_vec[w]
for i=0, nvel-1 do begin
    ...
    print, vsini
        ...
        fake_data = rt_make_data(amp_scaled, ctr, sig, vsini, noise=noise)
    
        a = min(fake_data[34:36])
        b = max(fake_data[42:48])
        c = min(fake_data[49:51])
        co[*,i,w] = [a,b,c]
    
    plot, fake_data, title = 'vsini = ' + strtrim(vsini,2), charsize = 2
    oplot, replicate(a, n_elements(fake_data)), color=200
    oplot, replicate(b, n_elements(fake_data)), color=200
    oplot, replicate(c, n_elements(fake_data)), color=200
    wait, 0.01
        endfor
endfor
;window, 0, xsize=1400, ysize=1000
;plot, vsini_vec, co[0,*]/co[1,*], ps=6, yr= [0.5,1.1], xtitle="vsini (km/s)", charsize = 2
;oplot, vsini_vec, co[2,*]/co[1,*], color=200, ps=6
;oplot, vsini_vec, co[0,*]/co[2,*], color=99999, ps = 6
;oplot, vsini_vec, co[2,*]-co[0,*]+0.5, color=99999, ps = 4
;oplot, vsini_vec, co[1,*], ps=4

aa = co[0,*,*]
bb = co[1,*,*]
cc = co[2,*,*]

qx = ((cc/bb)-(aa/bb))/(aa/cc)
qy = rebin(vsini_vec, nvel, nstrength)
