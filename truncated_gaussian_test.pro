...
common amoeba_info, data, nnondet,thresh, xvec, nsamp
...
; non-detections
;nondet_ratio = nnondet/nsamp
;cdf_nondet = double(interpol(cdf, xvec, nondet_ratio))
...
;  nondet_prob = total(nondet_gauss)*delta_x
 ...
 bigl_vec = renorm*(1d0/(sqrt(2d0*!pi)*sigma))*exp(-1d0*((data-mu)^2d0)/(2d0*sigma^2d0))
 ; Add the likelihood
...
pro truncated_gaussian_test, nsamp=nsamp, mu=mu, sigma=sigma, thresh=thresh, result=outstr
...
common amoeba_info, data, nnondet, thresh1, xvec, nsamp1
thresh1 = thresh
nsamp1 = nsamp
