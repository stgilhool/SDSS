...
hist = histogram(vector, binsize=binsize, min=binmin)
...
old_det_idx = where(qage gt 2 and vsini gt 5, nodet)
 ...
 set_plot, 'ps'
 device, filename = "veq_vs_teff_agemulti_stepfit.eps"
 device, /color, bits=8
...
set_plot, 'ps'
device, filename = "veq_vs_teff_agemulti_clear3.eps"
device, /color, bits=8
device, xs=12,ys=10, /inches
loadct, 13
