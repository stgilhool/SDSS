...
if ps then begin
    psopen, 'recoverability_test4_lsftest.eps', /encapsulated, xs=10, ys=7, /inches, /color
    !p.multi=[0,5,3]
endif
...
foreach teff, teff_vec, tidx do begin
    ...
    for i = 0, nvsini - 1 do begin
        ...
        for step = 0, 4 do begin
            
            g_zero_idx = where(abs(vel_x_over) ge (step+1)*6d0)
            g_out = g_out_copy
            g_out[g_zero_idx] = 0d0
            ...
            plot, vel_x_over, g_out, xr=[-40,40]
            dxstop
            
            plot, sigvec*1d3, glog_fft, xr=[0,200], yr=[-10,-2]
            dxstop
