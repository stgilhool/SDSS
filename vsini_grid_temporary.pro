...
;return, chi2
if model_out then return, final_spec else return, dev
...
outpath = '/home/stgilhool/APOGEE/vsini_results/temp_plot/'
...
irtf_info = mrdfits('/home/stgilhool/APOGEE/master_info5.fits',1)
...
sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0 $
                and (aspcapflag and flag_dec) eq 0 and finite(irtf_info.feh_irtf), nsel)
...
for afil = 0, napofiles-1 do begin
    ...
    if finite(irtf_info[afil].feh_irtf) then begin
        pos = value_locate(double(pfeh), irtf_info[afil].feh_irtf)
    ...
    for teff_idx = teff_loop_idx[0], teff_loop_idx[-1] do begin
        ...
        for feh = feh_0, feh_1 do begin
            ...
            ;for v = 0, 2 do begin
                ...
                ; Get best fit model
                model_out = 1
                best_mod = model_observation(r)
                
                set_plot, 'ps'
                outplot = 'rfile' + strtrim(afil,2) + '.eps'
                outplotfile = outpath+outplot
                device, filename = outplotfile, /encapsulated

                device, xs = 9, ys=8, /inches
                device, /color, decomposed = 0
                loadct, 12
                
                wl_vec = (10d0^(apo_logwl_trunc))/(10d0^4)

                title_str = "Vsini Fitting with PHOENIX template | Teff: "+strtrim(teff*100,2)+ "K | [Fe/H]: "+pfeh[feh]+ " | Vsini: " + strtrim(long(vsini),2) + " km/s"

                plot, wl_vec, apo_obs, /xs, yr=[0.6,1.2], ytit="Intensity", title=title_str
                oplot, wl_vec,best_mod, color=200
                plot, wl_vec, apo_obs-best_mod, ps =3, yr=[-0.3, 0.3], /xs, xtit="Wavelength (microns)", ytit="Residuals"
