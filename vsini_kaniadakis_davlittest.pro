...
pro vsini_kaniadakis_davlittest, vis=vis
...
; Read in the davison data
davfile = '/home/stgilhool/APOGEE/vsini_literature/davison/spec_results_dav_teff_vsini.txt'

readcol, davfile, cat, catid, teff_dav, vsini_dav, sig_vsini_dav, format = 'A,L,F,F,F', skipline=1
...
sel_idx1 = where(teff_all gt 2950 and teff_all lt 3050 and vsini_all gt 0 and ((aspcapflag and flag_dec) eq 0), nsel)
;sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0, nsel)

sel_idx = where(teff_dav eq 3000, nsel)
...
teff = teff_dav[sel_idx]
vsini = vsini_dav[sel_idx]
...
plot, histx, histyasp, ps=10, yr = [0, (max(histy)>max(histyasp))]

oplot, histx, histy, ps=10, color=200, linestyle=2
dxstop
...
save, /variables, filename='vsini_kan_davtest.sav'

plot_the_data:

if skip_to_plot then restore, 'vsini_kan_davtest.sav'

; For some reason, the usual ps approach didn't work
;  set_plot, 'ps'
;  device, filename = 'vsini_kan_davtest.eps', /encapsulated
; device, /color, bits=8
...
mwrfits, outstr, 'vsini_kaniadakis_davtest.fits', /create
