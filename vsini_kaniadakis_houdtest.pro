...
pro vsini_kaniadakis_houdtest, vis=vis
...
; Read houd
houdfile = 'houdebine_table5.fits'
...
sel_idx1 = where(teff_all gt 3100 and teff_all lt 3400 and vsini_all gt 0 and ((aspcapflag and flag_dec) eq 0), nsel1)
;sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0, nsel)

sel_idx = where(finite(hstruct.vsini), nsel)
...
histx = vsini_xvec_midpt
histyasp = histogram(vsiniasp, min=0d0, nbins=n_elements(vsini_xvec_midpt) )
histy = histogram(vsini, min=0d0, nbins=n_elements(vsini_xvec_midpt) )
plot, histx, histyasp/total(histyasp, /double), ps=10, yr = [0,0.4], xtit = "Vsini", ytit = "Frequency", tit = "Normalized dist. of Vsini for Houdebine dM4/Teff 3100-3400K stars", charsize = 2.0;yr = [0, (max(histy)>max(histyasp))]

oplot, histx, histy/total(histy, /double), ps=10, color=200, linestyle=2
dxstop
