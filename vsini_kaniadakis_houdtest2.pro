...
pro vsini_kaniadakis_houdtest2, vis=vis
...
; Read in the reiners data
;reinfile = '/home/stgilhool/APOGEE/vsini_literature/reiners/reiners_vsini_full.fits'

;rstruct = mrdfits(reinfile, 1)
; Read houd
houdfile = 'houdebine_all.fits'
hstruct = mrdfits(houdfile,1)
...
sel_idx1 = where(teff_all gt 3600 and teff_all lt 3700 and vsini_all gt 0 and ((aspcapflag and flag_dec) eq 0), nsel1)
;sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0, nsel)

sel_idx = where(finite(hstruct.vsini) and hstruct.teff gt 3600 and hstruct.teff lt 3700, nsel)
...
sel_info = aspcap_info[sel_idx1]
teffasp = sel_info.teff
;teff_err = sel_info.teff_err
vsiniasp = sel_info.vsini

;teff = hstruct[sel_idx].teff
teff = hstruct.teff[sel_idx]
;vsini = hstruct[sel_idx].vsini
vsini = hstruct.vsini[sel_idx]



dxstop

maxvsini = max(vsini)>max(vsiniasp)
...
minvsini = 0d0
vsini_range = maxvsini+thresh-minvsini
oversamp=11
;d_vsini = 0.1d0
...
histx = vsini_xvec_midpt
histyasp = histogram(vsiniasp, min=0d0, binsize=1, nbins=n_elements(vsini_xvec_midpt) )
histy = histogram(vsini, min=0d0, binsize=1, nbins=n_elements(vsini_xvec_midpt) )
histyasp_norm = histyasp/total(histyasp, /double)
histy_norm = histy/total(histy, /double)

pl_ymax = max(histyasp_norm) > max(histy_norm)

plot, histx-0.5,histyasp_norm, ps=10, yr = [0,pl_ymax], xtit = "Vsini", ytit = "Frequency", tit = "Normalized dist. of Vsini for Houdebine dM2/Teff 3600-3700 stars", charsize = 2.0;yr = [0, (max(histy)>max(histyasp))]

oplot, histx-0.5, histy_norm, ps=10, color=200, linestyle=2
dxstop
...
save, /variables, filename='vsini_kan_houdtest.sav'

plot_the_data:

if skip_to_plot then restore, 'vsini_kan_houdtest.sav'

; For some reason, the usual ps approach didn't work
;  set_plot, 'ps'
;  device, filename = 'vsini_kan_reintest.eps', /encapsulated
; device, /color, bits=8
...
mwrfits, outstr, 'vsini_kaniadakis_houdtest.fits', /create
