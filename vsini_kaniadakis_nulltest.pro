...
pro vsini_kaniadakis_nulltest, vis=vis
...
min_boot = dblarr(nbins,100)
nstars_per_bin=dblarr(nbins)
...
for bin = 0, nbins-1 do begin
    ...
    for iter = 0, 99 do begin
        ...
        if n_elements(result) gt 1 then begin
            ...
            min_boot[bin,iter] = min(vsini_boot)
...
save, /variables, filename='vsini_kan_nulltest.sav'

plot_the_data:

if skip_to_plot then restore, 'vsini_kan_nulltest.sav'
...
set_plot, 'ps'
device, filename = 'vsini_kaniadakis_nulltest.eps', /encapsulated
device, /color, bits=8
device, xs = 13, ys= 8, /inches
loadct, 13
...
mwrfits, outstr, 'vsini_kaniadakis_nulltest.fits', /create
