...
pro vsini_kaniadakis_nulltest_snr, vis=vis
...
sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0 and ((aspcapflag and flag_dec) eq 0) and snr_all ge 150, nsel)
...
teff_hist = histogram(teff, binsize = binsize, min = min_temp, max=max_temp, reverse_indices=ri)
;teff_hist = histogram(teff, binsize = binsize, min = min_temp, reverse_indices=ri)

nsel_after = total(teff_hist)
help, nsel_after
dxstop
...
nondet_prob_arr = dblarr(nbins, 100)
nondet_expect_arr = dblarr(nbins, 100)

mindet_prob_arr = dblarr(nbins, 100)
below_expect_arr = dblarr(nbins,100)

nondet_data_arr = dblarr(nbins)
...
for bin = 0, nbins-1 do begin
    ...
    for iter = 0, 99 do begin
        ...
        if n_elements(result) gt 1 then begin
            ...
            nondet_prob_arr[bin,iter] = prob_analysis[0]
            nondet_expect_arr[bin,iter] = prob_analysis[1]
            mindet_prob_arr[bin,iter] = prob_analysis[2]
            below_expect_arr[bin,iter] = prob_analysis[3]
...
save, /variables, filename='vsini_kan_nulltest_snr.sav'

plot_the_data:

if skip_to_plot then restore, 'vsini_kan_nulltest_snr.sav'
...
set_plot, 'ps'
device, filename = 'vsini_kaniadakis_nulltest_snr.eps', /encapsulated
device, /color, bits=8
device, xs = 13, ys= 8, /inches
loadct, 13
...
; fit a line
teff_fit_xvec = maken(min_temp,max_temp,3000)
...
; cgcontrol, output='cgkan_test.eps', IM_width=1000
 ...
 plot, teff_bin_vec_ctr, vsini_mean, ps=6, xtit = "Teff", ytit = "Peak Vsini", $
   tit=eq_string, charsize=2.5, xrange=[min_temp,max_temp+1], /xs
 oplot, teff_fit_xvec, teff_fit_yvec
 oploterror, teff_bin_vec_ctr, vsini_mean, vsini_stddev, ps=6
...
mwrfits, outstr, 'vsini_kaniadakis_nulltest_snr.fits', /create
