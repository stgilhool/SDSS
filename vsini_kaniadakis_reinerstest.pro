...
pro vsini_kaniadakis_reinerstest, vis=vis
...
sel_idx1 = where(teff_all gt 3100 and teff_all lt 3300 and vsini_all gt 0 and ((aspcapflag and flag_dec) eq 0), nsel)
;sel_idx = where(teff_all gt 2600 and teff_all lt 4000 and vsini_all gt 0, nsel)

sel_idx = where(rstruct.spt eq 'M4.5', nsel)
...
teff = replicate(3200, nsel)
vsini = rstruct[sel_idx].vsini
...
plot, histx, histyasp/total(histyasp, /double), ps=10, yr = [0,0.4], xtit = "Vsini", ytit = "Frequency", tit = "Normalized distribution of Vsini for M4.5/Teff~3200K stars", charsize = 2.0;yr = [0, (max(histy)>max(histyasp))]
...
save, /variables, filename='vsini_kan_reintest.sav'

plot_the_data:

if skip_to_plot then restore, 'vsini_kan_reintest.sav'
...
mwrfits, outstr, 'vsini_kaniadakis_reintest.fits', /create
