...
pro vsini_kaniadakis_snr, vis=vis
...
;print, minmax(sel_info.snr)
;plot, sel_info.snr, ps=6
;stop
...
; min_snr = 1.3d0
; binsize = 0.2
; snr_hist = histogram(logsnr, binsize = binsize, min = min_snr, reverse_indices=ri)

min_snr = 0d0
binsize = 100
max_snr = 999d0
snr_hist = histogram(snr, binsize = binsize, min = min_snr, max = max_snr, $
                     reverse_indices=ri)

nbins = n_elements(snr_hist) 

snr_bin_vec_0 = lindgen(nbins)*binsize + min_snr
snr_bin_vec_1 = (lindgen(nbins)+1)*binsize + min_snr
snr_bin_vec_ctr = (lindgen(nbins)+0.5)*binsize + min_snr
...
for bin = 0, nbins-1 do begin
    ...
    if ri1 gt ri0 then begin
        ...
        ;bin_snr = logsnr[bin_idx]        
        bin_snr = snr[bin_idx]        
        ;bin_teff_err = teff_err[bin_idx]        
        bin_vsini = vsini[bin_idx]
        ; store all the data
	bin_str = {bin_num:bin, $
                   bin_snr:[snr_bin_vec_0[bin],snr_bin_vec_1[bin]], $
                   bin_idx:bin_idx, $
...
for bin = 0, nbins-1 do begin
    ...
    print, "SNR bin: " + strtrim(bin,2) + " finished."

endfor

save, /variables, filename='vsini_kan_snr2.sav'

plot_the_data:

restore, 'vsini_kan_snr2.sav'
...
co1_str = string(round(snr_fit_coeff[1]*1d3)/1d3, format='(D6.3)')
co0_str = string(round(snr_fit_coeff[0]*1d1)/1d1, format='(D4.1)')
...
eq_string = 'Vsini = '+co1_str+'*SNR + '+ co0_str + ' km/s'
...
set_plot, 'ps'
device, filename = 'vsini_kaniadakis_snrplot.eps', /encapsulated
device, /color, bits=8
device, xs = 13, ys= 8, /inches
loadct, 13
...
;  oploterr, snr_bin_vec_ctr, vsini_mean, vsini_stddev
 ...
 plot, snr_bin_vec_ctr, vsini_mean, ps=6, xtit = "SNR", ytit = "Peak Vsini", $
   tit = "Peak Vsini vs. SNR", charsize=2.5
 ;oplot, snr_fit_xvec, snr_fit_yvec
 oploterr, snr_bin_vec_ctr, vsini_mean, vsini_stddev
...
outstr = {snr_bin_ctr:snr_bin_vec_ctr, $
          vsini_mean:vsini_mean, $
          vsini_stddev:vsini_stddev}

mwrfits, outstr, 'vsini_teff_snr2.fits', /create
