...
pro vsini_kaniadakis_test, vis=vis, thresh=thresh, vsini_0=vsini_0, kappa=kappa, sigma=sigma, result_mean=result_mean, result_stddev=result_stddev, std_err=std_err
...
; Generate the sample of 1000 points from the distribution
vsini_sample =[]
terminate = 0
repeat begin
    vsini_test = randomu(seed, 1d3)*80d0
    
    prob_vsini_test = interpol(pdf, vsini_xvec, vsini_test)
    
    rej_test_prob = randomu(seed, 1d3)
    ...
    if nsamples ge 1000 then begin
        vsini_sample = vsini_sample[0:999]
        terminate = 1
